...
public class Servidor {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		biblioteca.add(new Libro("8921-23423-586-6", "Don Quijote de la Mancha", "Miguel de Cervantes", "27.99"));
		biblioteca.add(new Libro("232-0-23892-386-5", "El Hobbit", "J.R.R. Tolkien", "12.99"));
		biblioteca.add(new Libro("789-1-23893-854-3", "EL Gran Gatsby", "F. Scott Fitzgerald", "10.99f"));
		biblioteca.add(new Libro("234-3-11029-345-1", "El nio del pijama de rayas", "John Boyne", "18.99"));
		biblioteca.add(new Libro("983-0-39429-348-4", "Harry Potter y la Piedra Filosofal", "J.K. Rowling", "22.99f"));
		// añadimos un libro más con el mismo autor
		biblioteca.add(new Libro("983-0-39339-348-2", "Harry Potter y la Cámara Secreta", "J.K. Rowling", "19.99"));
		...
		try(ServerSocket serverSocket = new ServerSocket()) {
			...
			while(true) {
				...
				// opcion 3 seleccionada por el cliente
				} else if(opc.equals("3")) {
					// buscamos el libro por Autor haciendo llamada a la función correspondiente a tal tarea
					/**
					 * Datos del libro que serán devueltos al cliente
					 */
					String datosLibro = buscarPorAutor(buscar);
					if(!datosLibro.equals(""))
						System.out.println("SERVIDOR: Se ha encontrado el libro, se procede a enviar " + datosLibro + " al cliente");
					else 
						System.out.println("SERVIDOR: No se ha encontrado el libro, se procede a enviar la cadena noLibro al cliente al cliente");
				...
				// opcion 4 seleccionada por el cliente
				} else if(opc.equals("4")) {
					// añadimos el libro a la biblioteca
					System.out.println("SERVIDOR: El cliente quiere añadir " + buscar + " a la blibioteca");
					// creamos el stream de salida del servidor al cliente
					salida = new PrintStream(socketAlCliente.getOutputStream());
					if (addLibro(buscar)) {
						// mandamos el OK al cliente
						salida.println("OK");
					} else {
						// No damos el OK al cliente
						salida.println("NOK");
						System.out.println("SERVIDOR: El cliente no ha podido añadir el libro a la blibioteca");
					}
				}
	...
	/**
	 * método que busca libros en la biblioteca habiendo introducido un autor determinado. El separador o regrex utilizado para los libros es :::
	 * @param autor Autor por el cual se buscará el libro en el servidor
	 * @return cadena con la información completa de los libros o noLibro si no se encuentra ningún libro
	 */
	public static String buscarPorAutor(String autor) {
		String libros = "";
		for(Libro libro : biblioteca) {
			if(libro.getAutor().equals(autor))
				libros += ":::" + libro.dataLibro();
		}
		if(libros.equals(""))
			return "noLibro";
		else
	 ...
	 * @param data cadena con los datos del libro a añadir
	 * @return true si se ha podido añadir el libro a la biblioteca, false en caso contrario
	 */
	public static boolean addLibro(String data) {
		String[] dataLibro =  data.trim().split("/");
		biblioteca.add(new Libro(dataLibro[0], dataLibro[1], dataLibro[2], dataLibro[3]));
		System.out.println("SERVIDOR: El cliente ha podido añadir el siguiente libro a la blibioteca:\n");
		System.out.println("ISBN: " + dataLibro[0]);
		System.out.println("Titulo: " + dataLibro[1]);
		System.out.println("Autor: " + dataLibro[2]);
		System.out.println("Precio: " + dataLibro[3] + "€");
		return true;
	}
